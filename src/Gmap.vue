  ...
  <div>
      ...
      <v-row align="center">
        <v-col cols="12">
          <v-sheet elevation="10">
            ...
            <v-chip @click="isOpenOnly = !isOpenOnly"> isOpen </v-chip>
          </v-sheet>
        </v-col>
      </v-row>
    </div>
    ...
    >
      <div v-for="post in shops" :key="post.id">
        <gmap-custom-marker
          :marker="{
            lat: post.google_place.latitude,
            lng: post.google_place.longitude,
          }"
        >
          <v-img
            class="img"
...
export default {
    ...
    shops() {
      ...
      if (this.isOpenOnly) {
        return this.posts.filter((post) => {
          ...
          let openHours = openTime[thisWeekday].time
            .split("")
            .slice(0, 2)
            .join("");
          let openMinutes = openTime[thisWeekday].time
            .split("")
            .slice(2)
            .join("");
          let closeHours = closeTime[thisWeekday].time
            .split("")
            .slice(0, 2)
            .join("");
          let closeMinutes = closeTime[thisWeekday].time
            .split("")
            .slice(2)
            .join("");
  ...
  methods: {
    ...
    getFeed(userId = null) {
      if (userId) {
        this.loading = true;
        axios.get("/feeds/?user_ids=" + userId).then((resp) => {
          this.posts = resp.data;
          this.loading = false;
        });
      } else {
        axios.get("/feeds/").then((resp) => {
          this.posts = resp.data;
        });
      }
    },
    getCategory(categoriesName = null) {
      if (categoriesName) {
        axios.get("/feeds/?categories=" + categoriesName).then((resp) => {
          this.posts = resp.data;
        });
      } else {
        axios.get("/feeds/").then((resp) => {
    ...
    getCityState(cityStatesName = null) {
      if (cityStatesName) {
        axios.get("/feeds/?city_state=" + cityStatesName).then((resp) => {
          this.posts = resp.data;
        });
      } else {
        axios.get("/feeds/").then((resp) => {
